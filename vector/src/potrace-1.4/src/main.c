...
static int usage(FILE *f) {
  ...
  if (have_compress()==1) {
    ...
    fprintf(f, " -2, --level2               - use postscript level 2 compression (disabled)\n");
  }
#ifdef HAVE_ZLIB
...
static int dopts(int ac, char *av[]) {
  ...
  while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
    ...
    case 'h':
      ...
      break;
    case 'v':
      fprintf(stdout, ""POTRACE" "VERSION". Copyright (C) 2001-2004 Peter Selinger.\n");
      exit(0);
      break;
    case 'l':
      fprintf(stdout, ""POTRACE" "VERSION". Copyright (C) 2001-2004 Peter Selinger.\n\n");
	...
	fprintf(stderr, ""POTRACE": option -2 not supported due to patent restrictions, ignoring.\n");
	info.pslevel = 2;
	info.compress = 0;
      }
      break;
    case '3':
      ...
      info.compress = 1;
#else
      fprintf(stderr, ""POTRACE": option -3 not supported, ignoring.\n");
#endif
      break;
    case 'e':
      backend_lookup("eps", &info.backend);
